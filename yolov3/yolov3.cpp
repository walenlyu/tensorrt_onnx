...
#define USE_FP16  # FP32 FP16 INT8
// stuff we know about the network and the input/output blobs
static const int INPUT_H = 608;
static const int INPUT_W = 608;
static const int BBOX_SIZE = 22743;
static const int NUM_CLASS = 80;
static const int OUTPUT_SIZE_1 = 22743*80;
static const int OUTPUT_SIZE_2 = 22743*4;
static const float SCORE_THRESH = 0.25;
static const float IOU_THRESH = 0.4;

const char* INPUT_BLOB_NAME = "images";
const char* OUTPUT_BLOB_NAME_1 = "classes";
const char* OUTPUT_BLOB_NAME_2 = "boxes";
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    std::string onnx_file = "../yolov3_608.onnx";
...
void doInference(IExecutionContext& context, float* input, float* output1, float* output2, int batchSize)
{
    ...
    CHECK(cudaMalloc(&buffers[outputIndex_1], batchSize * OUTPUT_SIZE_1 * sizeof(float)));
    CHECK(cudaMalloc(&buffers[outputIndex_2], batchSize * OUTPUT_SIZE_2 * sizeof(float)));
    ...
    CHECK(cudaMemcpyAsync(output1, buffers[outputIndex_1], batchSize * OUTPUT_SIZE_1 * sizeof(float), cudaMemcpyDeviceToHost, stream));
    CHECK(cudaMemcpyAsync(output2, buffers[outputIndex_2], batchSize * OUTPUT_SIZE_2 * sizeof(float), cudaMemcpyDeviceToHost, stream));
...
int main(int argc, char** argv)
{
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("yolov3_608.trt", std::ios::binary);
        if (!p)
        {
        ...
        std::ifstream file("yolov3_608.trt", std::ios::binary);
        if (file.good()) {
    ...
    // float out1 [OUTPUT_SIZE_1];
    // float out2 [OUTPUT_SIZE_2];
    float* data = new float[3 * INPUT_H * INPUT_W];
    float* out1 = new float[OUTPUT_SIZE_1];
    float* out2 = new float[OUTPUT_SIZE_2];
    ...
    for (auto f: file_names) {
        ...
        doInference(*context, data, out1, out2, 1);
        ...
        for( int i = 0; i < BBOX_SIZE; i++){
            ...
            for(int j = 0; j < NUM_CLASS; j++){
                if(out1[i*NUM_CLASS+j] > score){
                    score = out1[i*NUM_CLASS+j];
                    index = j;
                }
            }
            ...
            float x = out2[i*4+0]*INPUT_W ;;
            float y = out2[i*4+1]*INPUT_H ;
            float w = std::min(out2[i*4+2]*INPUT_W,(float)INPUT_W);
            float h = std::min(out2[i*4+3]*INPUT_H, (float)INPUT_H);
