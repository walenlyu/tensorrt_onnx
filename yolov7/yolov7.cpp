...
static const int OUTPUT_SIZE_1 = 25200*4;
static const int OUTPUT_SIZE_2 = 25200*80;
static const float SCORE_THRESH = 0.45;
static const float IOU_THRESH = 0.45;

const char* INPUT_BLOB_NAME = "images";
const char* OUTPUT_BLOB_NAME_1 = "boxes";
const char* OUTPUT_BLOB_NAME_2 = "classes";
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    std::string onnx_file = "../yolov7.onnx";
...
int main(int argc, char** argv)
{
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("yolov7.trt", std::ios::binary);
        if (!p)
        {
        ...
        std::ifstream file("yolov7.trt", std::ios::binary);
        if (file.good()) {
    ...
    for (auto f: file_names) {
        ...
        for( int i = 0; i < BBOX_SIZE; i++){
            ...
            for(int j = 0; j < NUM_CLASS; j++){
                if(out2[i*NUM_CLASS+j] > score){
                    score = out2[i*NUM_CLASS+j];
                    index = j;
                }
            }
            ...
            float x = (out1[i*4+0] + out1[i*4+2]) / 2.f;
            float y = (out1[i*4+1] + out1[i*4+3]) / 2.f ;
            float w = out1[i*4+2]- out1[i*4+0];
            float h = out1[i*4+3] - out1[i*4+1];
